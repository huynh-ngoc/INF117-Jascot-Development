          ...
          >
            <Card className="flex flex-col h-full transition-transform shadow-lg hover:scale-103">
              <CardHeader className="flex flex-col items-center justify-center space-y-2 p-4">
                ...
                <h2 className="text-2xl font-bold text-center">{report.state}</h2>
              </CardHeader>
              ...
              <CardContent className="flex-1 space-y-4">
                  ...
                  <ul className="space-y-1">
                    {report.why.map((w, i) => (
                      <li key={i} className="flex items-start space-x-2">
                        ...
                        <div className="mt-2">
                          <span className="flex-1 text-sm leading-snug">{w}</span>
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                <section className="grid content-normal gap-4">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <Stat icon={<DollarSign />} label="Median Price" value1={report.stats.medianPrice} />
                    <Stat icon={<Percent />} label="Cash-on-Cash" value1={report.stats.cashOnCash} />
                    <Stat icon={<AlertTriangle />} label="Crime Level" value1={report.stats.crime} />
                    <Stat icon={<BarChart2 />} label="Walkability" value1={report.stats.walkability} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <Stat icon={<DollarSign />} label="Avg Rent" value2={report.stats.avgRent} />
                    <Stat icon={<Percent />} label="Cap Rate" value2={report.stats.capRate} />
                    <Stat icon={<TrendingUp />} label2="3-yr Growth" value2={report.stats.popGrowth} />
                    <Stat icon={<Users />} label2="School Score" value2={report.stats.schoolScore} />
                  </div>
                </section>

                <section className="space-y-1">
                  <h3 className="text-lg font-semibold">Investor Grade</h3>
                  <Badge className="h-15 w-15 bg-slate-700 dark:bg-slate-400 text-2xl font-bold p-2">{report.investorGrade}</Badge>
                </section>
...
function Stat({ icon, label, label2, value1, value2 }) {
    return (
      <div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg shadow-sm">
        ...
        <div>
          <div className="text-2xl font-extrabold text-gray-900 dark:text-white">{value1}</div>
          <div className="text-xl font-extrabold text-gray-900 dark:text-white">{value2}</div>
          <div className="text-xs text-muted-foreground dark:text-gray-300">{label}</div>
          <div className="text-[10px] text-muted-foreground dark:text-gray-300">{label2}</div>
        </div>
      </div>
    );
  }